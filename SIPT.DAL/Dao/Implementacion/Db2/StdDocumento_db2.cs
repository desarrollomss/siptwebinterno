    ...
    public class StdDocumento_db2 : StdDocumento_dao
    {
        ...
        public override List<StdDocumento_InsertaResultado> Insertar(StdDocumento_InsertaDocWebSipt stdDocumento_InsertaDocWebSipt)
        {
            ...
            try
            {
                ...
                dataReader = DB2helper.ExecuteReader((DB2Transaction)this.dbconex.Transaccion(), CommandType.StoredProcedure, "SATT.SISDOC_INSERTA_DOCWEBSIPT", arrParam);
                stdDocumento_InsertaResultado = ConvertidorUtil.DeReaderAColeccion<StdDocumento_InsertaResultado, List<StdDocumento_InsertaResultado>>(dataReader);

                Log.Info(this.logMensajes.codigoMensaje.ToString(), "SATT.SISDOC_INSERTA_DOCWEBSIPT");
                Log.Debug(this.logMensajes.codigoMensaje.ToString(), arrParam);
                Log.Debug(this.logMensajes.codigoMensaje.ToString(), stdDocumento_InsertaResultado);
            }
            catch (Exception ex)
            {
                Log.Error(this.logMensajes.codigoMensaje.ToString(), "SATT.SISDOC_INSERTA_DOCWEBSIPT : " + ex.Message);
                Log.Error(this.logMensajes.codigoMensaje.ToString(), arrParam);
                throw ex;
            }
            return stdDocumento_InsertaResultado;
        }
